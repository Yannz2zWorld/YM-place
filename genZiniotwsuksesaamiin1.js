...
(function () {
  ...
  /* requireLogin — wajib login sebelum melanjutkan aksi */
  function requireLogin(callback) {
    if (currentUser) {
      callback();
    } else {
      showToast('🔒', 'Login dulu untuk membeli produk!');
      showAuthModal('login', callback);
    }
  }
...
${p.badge === 'recommend' ? '<span class="card-badge badge-recommend">Recommend</span>' : ''}
        ...
        <div class="card-footer">
          ...
          ${p.comingSoon
            ? `<button class="btn btn-disabled btn-sm btn-full card-buy" disabled style="opacity:0.5;cursor:not-allowed;background:var(--muted2)">🔒 Coming Soon</button>`
            : `<button class="btn btn-primary btn-sm btn-full card-buy" data-id="${p.id}">${currentUser ? 'Beli' : '🔒 Beli'}</button>`
          }
        </div>
      `;
  ...
  function showProductDetail(p) {
      ...
      <div class="modal">
        ...
        <div class="modal-body">
          ...
          <div class="modal-btns">
            ${p.comingSoon
              ? `<button class="btn btn-disabled" disabled style="opacity:0.5;cursor:not-allowed;flex:1">🔒 Coming Soon</button>`
    ...
    if (!p.comingSoon) {
      $('mdBuy').onclick = () => { closeOverlay(); requireLogin(() => showCheckout(p)); };
      $('mdCart').onclick = () => { requireLogin(() => { addToCart(p, 1); closeOverlay(); }); };
    }
    $('mdWish').onclick = () => {
      toggleWishlist(p.id);
  ...
  function renderWishlistSidebar() {
    ...
    wishlist.forEach(id => {
      ...
      `;
      item.querySelector('[data-wish-buy]').onclick = () => { closeWishlist(); requireLogin(() => showCheckout(p)); };
    ...
    openOverlay(`
      <div class="invoice-modal">
        ...
        <div class="invoice-body">
          ...
          <div class="invoice-row"><span>Metode</span><span>QRIS AUTO</span></div>
          <div class="invoice-row"><span>Status</span><span style="color:#22c55e;font-weight:700">Selesai</span></div>
